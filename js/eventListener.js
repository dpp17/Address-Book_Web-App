...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    const EmailID_Error = document.querySelector('.email-error');
...
// const save = (event) => {
//     let confirm = confirm("Confirm to Add");
//     try {
//         setAddressBookObj();
...
const save = (event) => {
    try {
        ...
        createAndUpdateStorage();
        resetForm();
        const confirmation = confirm("Address book entry added/updated successfully! Do you want to view the address book?");
        if (confirmation) {
            window.location.replace(site_properties.address_book_page);
        } else {
            window.location.replace(site_properties.home_page);
        }
    } catch (e) {
